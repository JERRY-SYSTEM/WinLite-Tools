...
::过滤精简列表
SET filter1="s/.*<Install package=//g;s/.*<Unproject package=//g;s/ update=.*>.*//g;s/~.*//g"
::过滤完整列表
SET filter2="s/.*<Install package=//g;s/.*<Unproject package=//g;s/ update=.*>.*//g"
::删除重复
SET filter3="$!N; /^\(.*\)\n\1$/!P; D"
::删除空格
...
for /f "delims=" %%i in ('FINDSTR /c:"Install package=" /c:"Unproject package=" %spath%\Sessions.xml 2^>nul^|%sed% %filter2% 2^>nul^|sort 2^>nul') do (ECHO %%~i >>%~dp0Package\Find-Full-Packages.txt)
%sed% -i %filter3% %~dp0Package\Find-Full-Packages.txt 1>NUL 2>NUL
%sed% -i %filter4% %~dp0Package\Find-Full-Packages.txt 1>NUL 2>NUL
ECHO.
ECHO 完整功能组件列表已生成。
ECHO 在 Package 文件夹
ECHO 文件名：Find-Full-Packages.txt
TIMEOUT 3 >NUL
GOTO:menu
...
for /f "delims=" %%i in ('FINDSTR /c:"Install package=" /c:"Unproject package=" %spath%\Sessions.xml 2^>nul^|%sed% %filter1% 2^>nul^|sort 2^>nul') do (ECHO %%~i >>%~dp0Package\Find-Lite-Packages.txt)
%sed% -i %filter3% %~dp0Package\Find-Lite-Packages.txt 1>NUL 2>NUL
%sed% -i %filter4% %~dp0Package\Find-Lite-Packages.txt 1>NUL 2>NUL
ECHO.
ECHO 精简功能组件列表已生成。
ECHO 在 Package 文件夹
ECHO 文件名：Find-Lite-Packages.txt
TIMEOUT 3 >NUL
GOTO:menu
